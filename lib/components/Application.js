...
export default class Application extends Component {
  ...
  appendMessage(msg){
    ...
    // append new message to messages const
    const newMessages = messages.concat([msg]);
    // set state to new messages
    this.setState({ messages: newMessages });
  }
  render(){
    return (
      <div>
        <p id="character-counter-output">Count</p> //move into new-message
        //because new message keeps track of the state of the form
        <NewMessage
          submitMessage={msg => this.appendMessage(msg)}
        />
        <ChatMaster />
        <ul>
